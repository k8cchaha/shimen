$(document).ready(function() {
    ...
    var cameraList = [];
    var timer = null;
    var timerDuration = 5*60*1000;

    // init
    var c_userName = getCookie('username');
    var c_login = getCookie('login');
    if (c_userName != null && c_userName != "" && c_login == "1") {
        $("#loginWnd").hide();
        $("#mainWnd").show();
        startTimer();
    } else {
    ...
    $("#loginBtn").bind("click", function() {
        ...
        $("#inputPassword").val("");
        setCookie('login', '1');
        setCookie('username', inputAccount);
    ...
    function startTimer() {
        stopTimer()
        timer = setInterval(function(){getAllInfo();}, timerDuration);
        getAllInfo();
    }
        ...
        for (var i = 0; i < camList.length; i++) {
            ...
            if (camList[i]["brand"] === "axis") {
                ...
                html += "<OBJECT ID='MyActiveX" + i + "' WIDTH=" + videoWidth + " HEIGHT=" + videoHeight + " CLASSID='CLSID:745395C8-D0E1-4227-8586-624CA9A10A8D' " +
                        "codebase='AMC.cab' border='1'>" + "<param name='AutoStart' value='0'>" + "<param name='UIMode' value='ptz-absolute'>" +
                ...
                html += "<OBJECT ID='MyActiveX" + i + "' WIDTH=" + videoWidth + " HEIGHT=" + videoHeight + " CLASSID='CLSID:36299202-09EF-4ABF-ADB9-47C599DBE778' " +
                        "codebase='myactivex.cab#version=1,0,0,1' border='1'></OBJECT>";
            }
    ...
    function initMenu() {
        ...
        $(".devAction").bind("click", function(){
            ...
            if (id) {
                ...
                if (tmp.length === 4) {
                    k = parseInt(tmp[3], 10);
                    handleAction(devMenu[i].content[j].content[k].act, devMenu[i].content[j].content[k].param, false);
                } else if (tmp.length === 3) {
                    handleAction(devMenu[i].content[j].act, devMenu[i].content[j].param, false);
                }
            }
        });
    }
    ...
    function initPoints() {
        ...
        $(".site").mouseover(function() {
            ...
            html += mapPoints[index].name + "<br><div>";
            for (var j = 0; j < mapPoints[index].content.length; j++) {
                menuID = "menu" + j;
                html += "<span id='" + menuID + "'>" + mapPoints[index].content[j].name + "</span><br>";
            }
            html += "</div>";
            $("#siteMenu").append(html);
    ...
    function loadJsonFile(url, callback) {
        $.ajax({
            ...
            success: function(data) {
                console.log("load json: " + url + " success!");
                callback(data);
            }
        })
    }
